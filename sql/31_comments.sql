...
COMMENT ON COLUMN curated.wells.wellstick_geom IS 'LINESTRING (4326) through Surface Hole -> Landing Point -> Midpoint -> Bottom Hole, natural traverse order. Vertex precedence Novi WellDetails -> Novi Wells -> Enverus (SHL/BHL); a Novi BHL/MP exactly equal to the Novi SHL is a vendor placeholder (skipped; BHL falls to Enverus). NULL when fewer than two DISTINCT vertices remain - guaranteed non-degenerate since sql/32 (2026-07). Cast to geography only via the sql/26 expression GiST indexes.';
...
COMMENT ON COLUMN curated.wells_enriched.wellstick_geom IS 'LINESTRING (4326) through Surface Hole -> Landing Point -> Midpoint -> Bottom Hole, natural traverse order. Vertex precedence Novi WellDetails -> Novi Wells -> Enverus (SHL/BHL); a Novi BHL/MP exactly equal to the Novi SHL is a vendor placeholder (skipped; BHL falls to Enverus). NULL when fewer than two DISTINCT vertices remain - guaranteed non-degenerate since sql/32 (2026-07). Cast to geography only via the sql/26 expression GiST indexes.';
...
COMMENT ON COLUMN curated.wells_enriched.stagger_closer_tangent_ft IS 'Novi WellSpacing StaggerCloserTangent: lateral (tangent) stagger distance (ft) to the closest stacked neighbor, AS-OF-FIRST-PRODUCTION. SENTINEL: exactly 2973.21 (~17% of rows; real max ~2,639) = no stacked neighbor, NOT a measurement. sql/48.';
COMMENT ON COLUMN curated.wells_enriched.parent_days_online IS 'Novi WellSpacing ParentDaysOnline: days the parent well(s) had been producing when this well came online. -1 exactly on the IsChild = FALSE / parent_count = 0 rows (verified 2026-09-18) - a flag value, not a duration. sql/48.';
...
-- =============================================================================
-- 31 (part H) -- curated.codev_context (sql/46) + curated.pdp_support_for_geom
-- (sql/47). Matview/function-level comments live in their own files; the

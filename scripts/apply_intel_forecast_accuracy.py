...
def main() -> None:
    t = time.monotonic()
    print("[1/4] geography indexes (sql/26, idempotent — required before sql/38)", flush=True)
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            print(f"  live-vintage recognition cliff: {cliff}", flush=True)
            print("  sql/38 tier x basin (distinct wells):", flush=True)
            ...
            print("  sql/38 wells with >= m aligned months:", flush=True)
            for m in (3, 6, 9, 12):
                n = cur.execute(
            ...
            for rv, m, n in cur.execute(
                ...
                "WHERE mop >= m AND pct_err_oil_perft IS NOT NULL "
                "GROUP BY 1,2 ORDER BY 1,2"
            ).fetchall():
                print(f"    {rv} mop>={m:2}  {n}", flush=True)
            bad = cur.execute(
                "SELECT COUNT(*) FROM curated.intel_forecast_accuracy_vintage "
                "WHERE tier='direct' AND pct_err_oil IS NOT NULL "
                "  AND ll_ratio IS NOT NULL AND pct_err_oil_perft IS NOT NULL "
                "  AND abs(pct_err_oil_perft - ((1 + pct_err_oil)/ll_ratio - 1)) > 1e-9"
            ).fetchone()[0]
